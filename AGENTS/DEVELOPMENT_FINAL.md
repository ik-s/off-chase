...
- 직접 테스트 요청의 `policyTamper: true` 옵션은 3,000 USDC 초과, 당시 한도 4,000 USDC 미만 요청에만 허용한다(기본 3,500). 기관의 원본 REJECT/KYT_RISK 응답을 독립 보관한 뒤, 사후 제시 사본의 정책 한도를 3,000으로 낮추고 거절 사유도 LIMIT_EXCEEDED로 바꿔 한도 초과였다고 주장하는 조작을 재현한다. 변경본에는 새 서명을 만들지 않는다. 원본 4,000 정책·요청·거절 응답·Anchor는 보존하며 변경본의 정책 및 응답 서명·해시 불일치를 실제 Verifier가 TAMPERED로 검출한다. 정상적인 새 버전 정책 발행과 구분하며, 기존 사건의 한도는 저장된 원본·제시본에서 표시한다.
