...
-- Display metadata only: signed records and their hashes remain unchanged.
lock table public.requests in access exclusive mode;
create sequence public.request_case_number_seq;
alter table public.requests add column case_number bigint;
with numbered as (
  select request_id, row_number() over (order by (record->>'created_at')::timestamptz, request_id) as number
  from public.requests
)
update public.requests r set case_number = n.number from numbered n where r.request_id = n.request_id;
select setval('public.request_case_number_seq', coalesce(max(case_number), 0) + 1, false) from public.requests;
alter sequence public.request_case_number_seq owned by public.requests.case_number;
alter table public.requests alter column case_number set default nextval('public.request_case_number_seq');
alter table public.requests alter column case_number set not null;
alter table public.requests add constraint requests_case_number_unique unique (case_number);
grant usage, select on sequence public.request_case_number_seq to service_role;
...
begin
  ...
  if not exists (select 1 from requests where request_id = v_request_id) then
