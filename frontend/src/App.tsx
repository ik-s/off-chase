import { TestRequest } from './TestRequest.tsx';
import { useEffect, useRef, useState } from 'react';
import type { CaseFilter, EvidenceRepository } from './data/types.ts';
import { CaseList, VerificationResult } from './components.tsx';
...
export default function App({ repository }: { repository: EvidenceRepository }) {
  const { state, dispatch, refreshList, retryDetail, submitTestRequest, runProgress, selectFile, verifyFile, download, verifyCase } = useWorkspace(repository);
  const [panel, setPanel] = useState<'home' | 'cases' | 'file' | 'test' | null>('home');
  ...
  return <div className="guided-app">
    ...
    <header className="flow-header">
      ...
      <div className="flow-header-tools">{repository.submitTestRequest && <button className="text-button" aria-pressed={panel === 'test'} onClick={() => setPanel('test')}>테스트 요청{state.demo ? ' · 진행 중' : ' +'}</button>}{mock && <span className="mock-tag">MOCK DEMO</span>}<button className="text-button" aria-pressed={panel === 'file'} onClick={() => setPanel('file')}>증거 파일로 검증 <span aria-hidden="true">↗</span></button></div>
    </header>
    <main id="main-content" className={`flow-main ${panel === 'home' ? 'home-layout' : ''}`}>
      ...
      <div ref={flowContent} hidden={panel !== null}>
        <div className="flow-context"><span><span className="eyebrow">CASE REVIEW</span><span className="context-case mono">{state.detail?.displayId ?? state.cases.find(item => item.id === state.selectedId)?.displayId ?? '요청 선택'}</span></span><button className="text-button" onClick={() => setPanel('cases')}>다른 요청 선택 <span aria-hidden="true">↗</span></button></div>
        ...
        {state.detail && <GuidedCase key={state.selectedId} detail={state.detail} mock={mock} onVerify={verifyCase} onDownload={() => void download()} downloading={state.downloadBusy} />}
      </div>
      {panel === 'cases' && <section className="flow-picker" aria-label="요청 선택">
        ...
        {state.listLoad === 'ready' && <div className="flow-actions"><button className="text-button" onClick={() => void refreshList()}>목록 새로고침</button></div>}
        {state.listError && <div className="error-message" role="alert">{state.listError}<button onClick={() => void refreshList()}>다시 시도</button></div>}
      </section>}
      {repository.submitTestRequest && <div hidden={panel !== 'test'}>
        <TestRequest headingRef={panel === 'test' ? panelHeading : undefined} busy={!!state.demo} run={runProgress} onSubmit={submitTestRequest}
          onOpen={id => { dispatch({ type: 'select', id }); setPanel(null); }} />
      </div>}
