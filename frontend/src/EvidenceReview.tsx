...
const requestChecks = ['schema', 'key', 'agent-signature', 'receipt-signature', 'request-hash', 'request-anchor'];
const policyChecks = ['policy-signature', 'policy-hash'];
...
function TechnicalValues({ values }: { values: [string, string][] }) {
  return <dl className="evidence-values">{values.map(([label, value]) => <div key={label}><dt>{label}</dt><dd><code>{value}</code></dd></div>)}</dl>;
}
...
function EvidenceSection({ title, description, ids, report, children, absent = false }: {
  ...
  const passed = !absent && checks.every(check => check.state === 'passed');
  const state = failed ? 'failed' : passed ? 'passed' : 'pending';
  const label = failed ? '확인 필요' : absent ? '응답 없음' : passed ? '확인됨' : '확인 대기';
  return <details className={`evidence-section evidence-${state}`}>
    <summary><span className="evidence-section-copy"><strong>{title}</strong><span>{description}</span></span><span className="evidence-section-status">{label}</span><span className="evidence-expand" aria-hidden="true">⌄</span></summary>
  ...
  return <div className="evidence-review">
    <section className="evidence-case-summary" aria-labelledby="evidence-case-heading">
      ...
      <dl className="evidence-response-strip">
        ...
        <div><dt>기업의 1회 결제 한도</dt><dd>{limit} <small>USDC</small></dd></div>
      ...
      {detail.change && <div className="evidence-warning"><strong>보관된 승인 기록이 현재 거절 기록으로 바뀌었습니다.</strong><p>기존 {detail.change.before} → 현재 {detail.change.after}</p><p>요청 {amount} USDC는 한도 {limit} USDC 이내입니다. 요청·정책·기존 Anchor는 유지되고 기관 응답만 변경된 모의 사례입니다.</p><p>KYT 위험의 진위가 아니라, 이미 보관된 판단과 현재 응답의 불일치를 탐지합니다.</p></div>}
    ...
    <EvidenceSection title="AI Agent가 보낸 결제 요청" description={`${amount} USDC 요청 · 요청자와 접수 기록 확인`} ids={requestChecks} report={report}>
      <Facts values={[[ '요청 ID', request.request_id ], ['요청 금액', `${amount} USDC`], ['요청 생성 시각', request.created_at], ['검증 계층의 접수 시각', utc(receipt.observed_at)]]} />
      ...
      <TechnicalValues values={[
        ['결제 수신 주소', request.recipient], ['AI Agent 키 ID', request.agent_key_id], ['AI Agent의 요청 서명', request.agent_signature],
    ...
    <EvidenceSection title="금융기관의 응답과 거절·승인 근거" description={decision ? `${decision.decision} · ${decision.reason_code} · 응답 서명과 보관 기록 비교` : '기관 응답 없음 · 결정 기한과 기록 누락 확인'} ids={responseChecks} report={report} absent={!decision}>
      <Facts values={[[ '기관 응답', decision?.decision ?? '없음' ], ['기록된 판단 사유', decision?.reason_code ?? '없음'], ['응답이 참조한 요청', decision?.request_id ?? '응답 없음'], ['결정 제출 기한', utc(receipt.decision_deadline)], ['응답의 온체인 등록 시각', detail.decisionAnchor ? utc(detail.decisionAnchor.timestamp) : '등록 기록 없음']]} />
