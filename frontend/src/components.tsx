...
export function DemoControls({ busy, onRun }: { busy: DemoScenario | null; onRun: (scenario: DemoScenario) => void }) {
  const demos: [DemoScenario, string, string][] = [['normal', 'Normal', '4,500 USDC · 한도 초과 거절'], ['tampered', 'Decision Tampered', '3,500 USDC · 승인 기록을 거절로 변경'], ['missing', 'Decision Missing', '3,500 USDC · 한도 이내 요청의 응답 누락'], ['deleted', 'Institution DB Deleted', '4,500 USDC · DB 삭제 후 거절 증거 유지']];
